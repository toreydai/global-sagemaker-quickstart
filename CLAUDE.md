...
- **Feature Store 离线库延迟（Demo16）**：Feature Group（online+offline）Creating→Created 约 20 秒；ingest 569 行 `failed_rows=[]` 即时写入在线库；但**离线库 parquet 约 5-6 分钟**才由后台批量写入 `.../offline-store/.../data/year=/month=/day=/hour=/`。ingest 后立即 `ls` 只见一个 0 字节 `.txt` 占位标记——判定落地须 `grep '.parquet$'`。离线 parquet 比在线多 3 个元数据列（`write_time`/`api_invocation_time`/`is_deleted`）。**清理注意**：`delete-feature-group` 不删离线 S3 数据，须显式 `s3 rm` 否则残留计费。
...
- **JumpStart 实例选型坑（Demo19）**：`mistral-7b-instruct`/`llama-3-8b-instruct` 等热门候选默认要 **4 卡 `ml.g5.12xlarge`**（多卡配额常为 0），且 Llama 系列 gated 需 `accept_eula=True`。选**非 gated、默认单卡实例**的模型（如 `huggingface-llm-falcon-7b-instruct-bf16` 默认 `ml.g5.2xlarge`）最容易在配额内跑通；部署实测约 **9.5 分钟** InService。TGI 推理返回 **JSON 数组**（`list`，非单 dict），取值需 `resp[0]["generated_text"]`；清理时 Model 名前缀是 `hf-llm-<model>-<ts>`，不含 endpoint 名，需按模型名关键字匹配删除。
